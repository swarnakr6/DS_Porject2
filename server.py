...
class PetAdoptionService(pet_adoption_pb2_grpc.PetAdoptionServiceServicer):
    def RegisterPet(self, request, context):
        ...
        return pet_adoption_pb2.RegisterResponse(message=f"Pet {request.name} registered successfully.")

    def SearchPet(self, request, context):
        results = [pet for pet in pets_db if request.query.lower() in (pet.name.lower(), pet.breed.lower(), pet.gender.lower())]
        return pet_adoption_pb2.SearchResponse(pets=results)

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    pet_adoption_pb2_grpc.add_PetAdoptionServiceServicer_to_server(PetAdoptionService(), server)
